...
#include "config/nexton-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "nexton.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "nextond.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 32698, 51474));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, nexton, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all NextON specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:32698"));
...
//    strUsage += "  -anonymizenextonamount=<n>     " + strprintf(_("Keep N PIV anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The NextON Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that NextON is running in a usable environment with all
...
#endif
    // Make sure only a single NextON process is using the data directory.
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //NextON: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //NextON specific: zerocoin and spork DB's
                ...
                // NextON: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // NextON: recalculate Accumulator Checkpoints that failed to database properly
...
//    }
//
//    nAnonymizeNextONAmount = GetArg("-anonymizenextonamount", 0);
//    if (nAnonymizeNextONAmount > 999999) nAnonymizeNextONAmount = 999999;
//    if (nAnonymizeNextONAmount < 2) nAnonymizeNextONAmount = 2;
    ...
    LogPrintf("Anonymize NextON Amount %d\n", nAnonymizeNextONAmount);
