...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total nexton balance of the wallet (excluding zerocoins)\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"nextonaddress\"\n"
            "\nReturn information about the given nexton address.\n"

            "\nArguments:\n"
            "1. \"nextonaddress\"     (string, required) The nexton address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"nextonaddress\", (string) The nexton address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: NextON address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are nexton addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) nexton address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"nextonaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"nextonaddress\"  (string, required) The nexton address to use for the signature.\n"
