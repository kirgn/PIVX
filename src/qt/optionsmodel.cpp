...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeNextONAmount"))
        settings.setValue("nAnonymizeNextONAmount", 1000);

    nAnonymizeNextONAmount = settings.value("nAnonymizeNextONAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNextONAmount"))
        SoftSetArg("-anonymizenextonamount", settings.value("nAnonymizeNextONAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nexton.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNextONAmount:
            return QVariant(nAnonymizeNextONAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNextONAmount:
            nAnonymizeNextONAmount = value.toInt();
            settings.setValue("nAnonymizeNextONAmount", nAnonymizeNextONAmount);
            emit anonymizeNextONAmountChanged(nAnonymizeNextONAmount);
            break;
        case CoinControlFeatures:
